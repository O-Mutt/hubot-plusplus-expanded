...
module.exports = (robot, scripts) => {
  ...
  databaseService.getMagicSecretStringNumberValue().then((databaseMagicString) => {
    const magicMnumber = Crypto.decrypt(procVars.magicIv, procVars.magicNumber, databaseMagicString);
    tokenBuddy.init({
      index: 0,
    ...
    robot.logger.error('Error getting magic string from database: ignore ', err);
  });

  const scriptsPath = path.resolve(__dirname, 'src');
  return fs.exists(scriptsPath, (exists) => {
    if (exists) {
      const scriptsToLoad = scripts && scripts.indexOf('*') < 0 ? scripts : null;
      const loadedScripts = loadScripts(robot, scriptsPath);

      // Filter out scripts that weren't loaded
      if (scriptsToLoad) {
        const missingScripts = scriptsToLoad.filter((script) => !loadedScripts.includes(script));
        if (missingScripts.length > 0) {
          console.error(`Hubot couldn't find the following scripts: ${missingScripts.join(', ')}`);
        }
      }
    }
  });
...
function loadScripts(robot, directory) {
  ...
  fs.readdirSync(directory).forEach((file) => {
    ...
    if (stats.isDirectory()) {
      ...
      if (exported instanceof Function && !exported.toString().startsWith('class')) {
