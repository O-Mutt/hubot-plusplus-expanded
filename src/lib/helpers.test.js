...
describe('Helpers', () => {
  describe('cleanName', () => {
    it('should clean "@matt" of the @ sign and be "matt" if @ is the first char', () => {
      ...
      expect(cleanName(fullName)).toBe(cleaned);
    });
  ...
  describe('cleanAndEncodeReason', () => {
    it('should clean and encode "toaster flish tons"', () => {
      const input = 'toaster flish tons';
      const expected = 'matt';

      expect(cleanAndEncode(input)).toBe(expected);
    });
  ...
  describe('decodeReason', () => {
    it('should decode the reason from base64 encoded "you are the best!" to "you are the best!"', () => {
      ...
      expect(decode(encoded)).toBe(cleaned);
    });
  ...
  describe('parseDateStrAndFormat', () => {
    it('should parse "2018-01-01" to a date object', () => {
      ...
      const result = parseDateStrAndFormat(dateStr);
